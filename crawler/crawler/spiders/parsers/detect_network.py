...
class DetectNetwork:
    network = {
        ...
        'Terra': {'chain_id': None, 'keys': ('LUNC', 'Terra Classic',)},
        'Terra 2.0': {'chain_id': None, 'keys': ('LUNA', 'Terra', 'Terra 2.0')},
        'Moonriver': {'chain_id': 1285, 'keys': ('Moonriver',)},
        'KCC': {'chain_id': 321, 'keys': ('KCC',)},
        'xDai': {'chain_id': 100, 'keys': ('xDai',)},
        'OKExChain': {'chain_id': 66, 'keys': ('OKExChain',)},
    ...
    def __call__(
        ...
        for k, v in self.network.items():
            if self._check_by_keys(key=k, name_network=name_network):
                return k, v['chain_id']
            if chain_id == v['chain_id'] and chain_id:
                return k, v['chain_id']
